...
- 앱 목적(미션): 초등학생도 이해할 만큼 쉬운 화면으로 수입·지출을 관리하면서, 그 기록이 복식부기 SSOT와 최신 법정서식·세법 근거로 뒷받침되어, 사용자가 세무사 없이도 스스로 국세청에 세무사 수준의 최신 법적 신고자료를 만들 수 있게 AI 역량을 총동원해 돕는다. 쉬움과 정확성 어느 하나도 포기하지 않는다. 상세는 `docs/accounting-ledger-design-directive-v2.md`의 `0. 최상위 결론`을 SSOT로 본다.
- 현재 앱 버전은 `0.05`다. Google OAuth owner 실로그인·초기 동기화, 연결 가이드, 인증 owner 기준 `businesses` CRUD·RLS 왕복 자가검증을 거쳐, `관리 → 데이터 관리` 화면에 저장·백업·복원·동기화·부분/일괄 삭제를 대칭 구조로 모았다. 다음 사용자 영향 변경은 `0.06`이다.
