...
- 버그는 한 건으로 끝내지 말고 클래스로 일반화해 하네스 게이트로 재발을 막는다. 중요한 불변조건은 문서 다짐이 아니라 CI 게이트로 강제한다(현재: version·latest-marker·lifecycle-matrix·logic-tests·term-ledger·legal-ssot). 세법 용어를 추가하면 `TAX_TERMS`와 `docs/accounting-ledger-term-ledger.md`를 함께 갱신해야 게이트를 통과한다.
