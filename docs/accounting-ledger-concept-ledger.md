...
| 원화 기능통화 + 환율 증거 고정 | 외화 원금액은 보존하되 장부·전표는 저장 당시 확정한 KRW 금액으로 만들고, 적용환율·기준일·출처를 함께 잠근다 | 오늘 환율로 과거 전표를 다시 계산하면 장부 숫자와 감사 근거가 바뀌고, 통화 기호만 바꾸면 실제 금액이 틀어진다. 고정지출 예상액과 실제 거래 확정액도 구분해야 한다 | `MoneyDomain` | `docs/skills/accounting-multicurrency-fx-skill.md` |
