> 기준일: 2026-07-11
> 앱 버전: `0.26`
> 상태: 가져오기 **검증·수정 모달**(0.26). 업로드 시 자동 오픈, 오류(날짜·금액)·경고(미분류 계정과목·부가세 총액 의심) 탐지, 수정 제안(계정과목 치환·공급가액 재계산)을 사용자 승인 후 적용. 실 샘플에서 부가세 의심 512건 탐지·일괄수정·945건 균형 반영 헤드리스 검증. **0.16~0.23 배포됨; 0.24~0.26은 브랜치**
...
| 최근 기준 커밋 | `f1bffc3 feat: post imported simple-book rows into double-entry ledger (idempotent) for app 0.25`. 앱 0.26 변경은 `claude/businesses-crud-rls-validation-v5dzbu` 브랜치 기준 |
