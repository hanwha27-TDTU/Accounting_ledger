> 기준일: 2026-07-11
> 앱 버전: `0.29`
> 상태: 해외 거래 구조화 저장(0.29). **마이그레이션 적용**(source_transactions에 is_overseas/foreign_currency/foreign_amount/exchange_rate 추가, additive·nullable, 프로덕션 BEFORE 0행/read-back 확인). 거래에 구조화 저장·장부 🌐 배지·Supabase 동기화 컬럼 준비. schema_version 0.04. **0.16~0.26 배포됨; 0.27~0.29는 브랜치(마이그레이션은 프로덕션 적용됨)**
...
| 최근 기준 커밋 | `ba60087 feat: overseas-transaction input (FX conversion + reverse-charge warning) for app 0.28`. 앱 0.29 변경은 `claude/businesses-crud-rls-validation-v5dzbu` 브랜치 기준 |
