> 기준일: 2026-07-12
> 앱 버전: `0.33`
> 상태: 개인 가계부 모드(0.33). 첫 실행 시 개인/사업자 모드 선택. 개인 모드는 사업자 등록·부가세·경비율·법정서식 화면 없이 24개 카테고리(배관용 6+개인 18)로 간편장부처럼 수입·지출·자산을 관리. 계정과목은 설정의 `계정과목 관리`에서 자유 추가/비활성화(사업자 모드도 공용). 설정에서 모드 전환 가능(데이터 유지). 사업자 모드는 완전 무변경(회귀 검증 완료). **0.16~0.30 배포됨; 0.31~0.33은 브랜치**
