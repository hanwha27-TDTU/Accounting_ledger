> 기준일: 2026-07-11
> 앱 버전: `0.27`
> 상태: 해외 구매·수입 처리 안내(0.27). 가이드 `overseas` 토픽 + 용어 3개(수입부가세·대리납부·기준환율)를 부가가치세법 §4·§52·소득세법 §27 근거로 추가. 면세사업자 대리납부 경고 강조. **0.16~0.26 배포됨; 0.27은 브랜치**
...
| 최근 기준 커밋 | `a90c6ec feat: import validation & fix modal (detect + suggest + approve) for app 0.26`. 앱 0.27 변경은 `claude/businesses-crud-rls-validation-v5dzbu` 브랜치 기준 |
