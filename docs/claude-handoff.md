> 기준일: 2026-07-11
> 앱 버전: `0.24`
> 상태: 간편장부 Excel 미리보기(#5 파싱+미리보기) 구현·실데이터 검증(0.24). 외부 라이브러리 없이 브라우저에서 .xlsx 파싱(ZIP+DecompressionStream+DOMParser). 실제 사용자 샘플 945건(2025, 수입 40,000,000·비용 38,625,037) 헤드리스 검증 완료. 다음: import 확정→원장 전기(중복 탐지). **0.16~0.23 배포됨; 0.24는 브랜치**
...
| 최근 기준 커밋 | `aa80685 chore: full audit of 0.16-0.23 + lock 추계 multipliers in legal-ssot gate`. 앱 0.24 변경은 `claude/businesses-crud-rls-validation-v5dzbu` 브랜치 기준 |
