> 기준일: 2026-07-11
> 앱 버전: `0.31`
> 상태: 월별 사용현황(0.31). 상단 공통 버튼 → 계정과목(열)×월(행) 사용금액 모달, 법정서식과 분리된 검토용. 간편장부·복식부기 관점 모두 같은 표로 커버. **0.16~0.30 배포됨; 0.31은 브랜치**
...
| 최근 기준 커밋 | `5a774a5 feat: currency picker (ISO 4217 datalist incl. UZS) for app 0.30`. 앱 0.31 변경은 `claude/businesses-crud-rls-validation-v5dzbu` 브랜치 기준 |
