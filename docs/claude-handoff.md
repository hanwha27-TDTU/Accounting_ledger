> 기준일: 2026-07-11
> 앱 버전: `0.32`
> 상태: 계정과목 선택 가이드(0.32). 가이드에 25개 계정과목(법적 정의+눈높이 설명+예시) + 구분법 팁(소모품비 vs 비품 등), 거래 입력 화면에 바로가기. 그 앞: 월별 사용현황(0.31). **0.16~0.30 배포됨; 0.31·0.32는 브랜치**
...
| 최근 기준 커밋 | `5ec4aa0 feat: monthly usage matrix (account x month review table) for app 0.31`. 앱 0.32 변경은 `claude/businesses-crud-rls-validation-v5dzbu` 브랜치 기준 |
