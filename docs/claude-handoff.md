> 기준일: 2026-07-11
> 앱 버전: `0.16`
> 상태: 입력 자동화 ②그룹 진행 — 경비율(0.15)에 이어 기장의무 기준(소득세법 시행령 §208⑤)과 부가세 면세 후보(부가가치세법 §26①)를 법령 원문 근거로 업종코드 선택 시 자동 표시(0.16)
...
| 최근 기준 커밋 | `4ff7a5c feat: NTS expense-rate display per industry code for app 0.15`. 앱 0.16 변경은 `claude/businesses-crud-rls-validation-v5dzbu` 브랜치 기준 |
...
0.14에서 입력 칸 자동화 검토를 시작했다. 사용자가 "법적 필요성 재검토 + 휴먼에러 방지 자동화"를 요청했고 자동화를 3그룹으로 분류했다. ①그룹(외부 자료 불필요)은 0.14 구현: 사업자번호 체크섬·대표자명 자동채움·큰업종 자동. ②그룹(사용자 자료 필요, 세무 정확성): ~~경비율 데이터~~(**0.15 완료**), ~~부가세 면세~~(**0.16 완료** — 부가가치세법 §26① 원문 근거, 업종 연결 호를 후보 표시), ~~기장의무 수입금액 기준표~~(**0.16 완료** — 소득세법 시행령 §208⑤ 원문 근거, NTS 대분류→가/나/다 그룹→기준금액), **간편장부 계정과목 코드표**(작성사례 PDF에서 112·120·121·123·125 등 3자리 코드 사용 확인), 경비율 **초과율 적용 수입금액 임계표**(초과율 자동계산에 필요). 0.16의 면세·기장의무는 법령이 SSOT이므로 별도 사용자 엑셀 없이 법령 MCP로 구현했다. ③그룹(무료 공공 API 키): juso.go.kr 주소검색, 공공데이터포털 사업자등록 상태조회(브라우저 CORS로 Edge Function 프록시 필요 가능). ②③은 자료·키 수령 후 진행하며 세무 항목은 후보(requires_review)+출처로 처리한다.
