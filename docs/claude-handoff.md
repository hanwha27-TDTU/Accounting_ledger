> 기준일: 2026-07-11
> 앱 버전: `0.25`
> 상태: 간편장부 가져오기 **원장 전기 완료**(#5 완성, 0.25). 미리보기 거래를 복식부기 원장에 반영 — 결정적 id로 재반영 시 중복 없음(좀비 방지), 계정과목·거래처 자동 생성, 균형 전표. 실 샘플 945건 헤드리스 검증(차변=대변, 재반영 시 945 유지). 다음: 사업자 편집·리포트 연동 등. **0.16~0.23 배포됨; 0.24·0.25는 브랜치**
...
| 최근 기준 커밋 | `9f21063 feat: simple-book Excel import preview (#5 parse+preview) for app 0.24`. 앱 0.25 변경은 `claude/businesses-crud-rls-validation-v5dzbu` 브랜치 기준 |
