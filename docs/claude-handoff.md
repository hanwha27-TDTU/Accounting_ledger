> 기준일: 2026-07-11
> 앱 버전: `0.19`
> 상태: 입력 자동화 ②그룹 진행 — 추계소득 엔진(0.18) 위에 **추계소득금액 계산기 UI**를 리포트에 추가(0.19). 총수입·매입·임차·인건비 입력 + 기장의무 선택 → 업종 경비율 자동 채움(수정 가능) → 소득금액 min(①,②) 실시간 계산. 헤드리스 브라우저로 작성사례 수치 검증. 다음: 간편장부 Excel import(#5)
...
| 최근 기준 커밋 | `bb8a2cf feat: estimated-income (standard expense-rate) engine from NTS worked example for app 0.18`. 앱 0.19 변경은 `claude/businesses-crud-rls-validation-v5dzbu` 브랜치 기준 |
