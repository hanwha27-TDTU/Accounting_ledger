> **Sub_multicurrency-fx_0.02** · 개정 2026-08-04
...
4. 고정지출 일정은 외화 원금액과 사용자가 선택한 환율 기준일의 적용 환율을 보존한다. 목록·대시보드 예상액은 이 저장 환율을 일관되게 사용하고, 실제 거래가 생성될 때 거래일의 환율을 별도로 확정한다.
...
- 기준일에는 그 날짜 이하의 가장 최근 공식 고시일을 사용하고 실제 `exchange_rate_date`를 표시·보존한다. 주말·공휴일은 직전 고시일을 쓰며, 미래 기준일은 오늘 이하 최신 고시환율을 사용한 예상치임을 명시한다.

## 실패·오프라인 규칙

1. 같은 요청일의 로컬 캐시가 있으면 재사용한다. 날짜별 캐시는 최근 항목만 제한적으로 보관한다.
2. 네트워크 실패 시 캐시를 쓰더라도 선택 기준일보다 뒤에 고시된 환율은 사용하지 않는다. 사용 가능한 마지막 정상 스냅샷에는 “마지막 정상 환율” 경고를 표시하고 거래를 검토 대상으로 남긴다.
...
- 외화 선택 시 원화 환산액, 선택 기준일, 실제 공식 고시일, 출처, stale/수동 상태를 즉시 보여준다.
- 환율은 수정 가능해야 하고 공식환율 새로고침 버튼을 제공한다.
- 고정지출 폼은 거래 예정일과 별개의 `환율 기준일`을 제공한다. 목록은 외화 원금액을 우선 보여주고 저장된 기준 환율의 `≈ 원화 · 실제 고시일 기준`을 보조 표시하며, 통화별 금액의 오른쪽 끝선을 맞춘다.
- 거래 목록·장부·전표·집계는 저장 당시 원화 확정액을 사용하고, 고정지출 요약은 저장된 기준 환율의 예상액을 사용한다.
...
- [ ] 고정지출의 선택 기준일·실제 고시일이 구분되고 목록·상단 요약이 저장된 기준 환율과 일치한다.
- [ ] 과거 기준일의 네트워크 실패 fallback이 그 기준일보다 미래인 캐시를 사용하지 않는다.
