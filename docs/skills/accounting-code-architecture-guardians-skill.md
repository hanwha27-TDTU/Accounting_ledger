...
1. 진단 도구의 그룹·도구·사각지대·판정은 등록부에서 화면과 보고서로 파생한다. 같은 문장을 화면마다 복제하지 않는다.
2. 판정은 `ok`, `todo`, `problem`, `unknown` 네 상태이며 우선순위는 `problem > unknown > todo > ok`다. 모집단 0은 정상으로 세지 않는다.
3. 모든 지표는 실제값과 기대값을 함께 갖고, `unknown`은 `structural` 또는 `transient` 종류와 이유를 반드시 밝힌다.
4. 모든 도구는 본 것·못 본 것·쓴 것을 표시한다. 허용 쓰기는 `none`, 기존 연결 진단 뒤의 `existing-sync`, 사용자 레코드와 격리된 `system-scratch`뿐이다.
5. 허브를 열거나 요약을 그리는 행위는 네트워크·IndexedDB·localStorage·운영 데이터를 바꾸지 않는다. 서버 요청은 사용자의 명시 실행에서만 한다.
6. 연결 진단·RLS 왕복·동기화 같은 기존 정본 서비스는 복제하지 않고 호출한다. 연결 성공 뒤 즉시 동기화하는 프로젝트 불변조건도 숨기지 않는다.
