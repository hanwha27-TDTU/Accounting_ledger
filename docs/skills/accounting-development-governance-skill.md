> **Sub_development-governance_0.10** · 개정 2026-08-07
...
이름이 낡아 보이거나 Advisor가 `unused`라고 표시했다는 이유만으로 자산을 삭제하지 않는다. 다음 순서로 폐기 가능성을 증명한다.

1. 후보를 브랜치 포인터, 로컬 임시·생성물, 런타임·스키마 자산으로 분류한다. 감사 증거와 오프라인 공급망 검증용 파생물은 재생성 가능하더라도 보존 근거를 먼저 확인한다.
2. 병합 브랜치는 커밋이 `main`의 조상인지 확인한다. squash merge라면 PR 병합 상태와 head SHA가 보존됐는지 확인한 뒤 포인터만 지운다.
3. 원격 테이블은 삭제 전에 행 수, FK·뷰·함수 참조, trigger, 정책, GRANT, 앱 데이터 경로를 각각 확인한다. `unused_index` 같은 통계성 알림은 삭제 증거로 쓰지 않는다.
4. 파괴적 migration은 데이터가 남아 있으면 예외로 중단하는 fail-closed guard를 둔다. `CASCADE`를 쓰지 않아 미확인 의존성이 삭제를 막게 하고, 최신 PR CI 성공 뒤 운영에 적용한다.
5. 운영 적용 뒤에는 대상 부재만 보지 말고 인접 schema version, 필수 RLS·RPC 권한, Advisor 변화도 되읽는다. migration 성공과 후속 read-back 쿼리 실패는 별도 결과로 기록한다.
6. 세션 종료 전 `main == origin/main`, 깨끗한 작업 트리, 병합 완료 작업 브랜치 부재, 최신 Quality·배포 표면 상태를 확인한다.
...
7. 반복 일정과 실제 거래를 분리하고, 실제 거래 저장·일정 전진을 같은 로컬 원자 작업으로 묶는다. 월간은 결제일(1~30일, 31=말일), 연간은 결제월·결제일을 기준값으로 저장하고 다음 납부 예정일은 파생한다. 조기 결제도 다음 회차로 전진하고, 재개 시 지난 일정을 건너뛰어 오늘 이후 가장 가까운 일정으로 맞추며, 거래 연결 ID를 보존한다.
...
| 반복 일정 | 월간 결제일·연간 결제월일 입력, 다음 예정일 자동 파생, 29/30/31일·윤년 보정, 조기 결제 1회 전진, 미납 catch-up, 재개 시 미래 일정 재계산, 실제 거래 연결, 중지·해지·삭제, 기기 간 수렴 |
