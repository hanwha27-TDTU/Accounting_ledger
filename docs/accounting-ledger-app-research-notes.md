...
| 보안 감사 | 기준 커밋 `5c84d99`를 대상으로 표준 감사를 봉인했다. 검증 10건(High 4·Medium 4·Low 2) 중 9건은 현재 작업 트리에서 보완했다: tenant-scoped tombstone/canonical meta, DB 역할 강제, audit RPC, 원격 `sync_queue` 차단 및 후속 퇴역, 백업 URL·행 검증, XLSX 자원 상한, 서명 APK fail-closed, Android 백업 차단. GitHub Pages 공유 origin 1건은 전용 custom origin이 필요해 잔여 위험으로 남긴다. |
| 데이터·배포 | 앱 표시 schema는 0.07, backup은 0.03이다. 운영 프로젝트 `ihxiywffzmvrwmqvatzt`에 tenant 보안 migration(`20260807113011`)과 tombstone owner FK 인덱스 migration을 적용했다. 원격 `sync_queue`는 행 0·FK 0·함수/뷰 참조 0·Data API grant 0을 재확인한 뒤 fail-closed migration(`20260807115909`)으로 퇴역했다. 퇴역 후 테이블 부재·schema 0.07·tombstone/RPC 권한 유지를 되읽었고 Advisor의 queue 무정책 RLS·미사용 인덱스 알림도 사라졌다. 웹·APK의 현재 배포 상태는 Git/Actions/공개 표면을 SSOT로 삼는다. 봉인된 Codex Security scan artifact를 감사 SSOT로 삼는다. |
| 불필요 자산 정리 | main/PR에 보존된 병합 브랜치 포인터, Supabase CLI 빈 `.temp`, 잘못된 템플릿 패키지의 Android 예제 테스트 2개를 폐기했다. 오프라인 공급망 검증용 vendor와 Actions 실행·artifact는 감사·재현 근거라 보존했다. |
...
## 2026-07-15 앱 0.47: 재산세 계산기 반기분납 금액 표시

| 항목 | 내용 |
|---|---|
| app_version | 0.46 → 0.47 |
| note_type | `feature`(계산 로직은 기존 `total`을 나누기만 함 — 새 세액 계산 없음) |
| 제목 | 재산세 계산기 결과에 실제 납부 시기(7월 1기분·9월 2기분)별 금액을 추가 |
| 배경 | 0.46 배포 완료 보고 후 사용자 "반기분납 개념도 적용하자 당신 생각은?" — 탐색적 질문이라 짧게 추천과 트레이드오프만 제시: 찬성(0.43에서 실제 고지서를 대조할 때 "1기분(상반기 절반)"이라 2배로 환산해야 했던 바로 그 지점이라 계산기에 넣으면 실제 체감과 더 가까워짐), 트레이드오프(20만원 이하 구간의 일괄징수 여부는 법이 지자체 조례에 위임한 부분이라 모든 지자체가 동일하게 운영한다고 단정할 수 없어 안내 문구가 필요함). 사용자 "진행하자" 승인. |
| 조사 | 지방세법 §115(납기)를 `get_law_text`로 원문 조회. ①3호: 주택분 재산세는 "해당 연도에 부과·징수할 세액의 2분의 1은 매년 7월 16일부터 7월 31일까지, 나머지 2분의 1은 9월 16일부터 9월 30일까지" 납부하되, "해당 연도에 부과할 세액이 20만원 이하인 경우에는 조례로 정하는 바에 따라 납기를 7월 16일부터 7월 31일까지로 하여 한꺼번에 부과·징수할 수 있다"(permissive, 지자체 조례 위임)는 것을 확인. 도시지역분은 §112에 따라 재산세액에 합산되어 같은 납기를 따르고, 지방교육세는 §152②에 따라 재산세·주민세 등과 "함께" 부과·징수되므로 역시 같은 납기를 따른다고 판단 — 그래서 세 세목을 합산한 최종 `total`(지분 반영값)을 절반으로 나누는 것이 올바른 접근이라고 결론. 20만원 기준이 지방교육세를 포함하는지 여부는 조문에서 명확히 확정할 근거를 찾지 못해, 이 앱은 그 판단을 자동으로 내리지 않고 항상 안내 문구로만 남겨 사용자가 관할 지자체에 확인하도록 했다(보수적 접근 — TAX_CALC_GUARDIANS 원칙). |
| 구현 | `PropertyTax.calculateHousing()`에 `LUMP_SUM_THRESHOLD`(20만원) 상수와 `firstInstallment`/`secondInstallment`/`lumpSumEligible` 반환값 추가 — 새 세액을 계산하지 않고 이미 계산된 `total`을 `Math.round(total/2)`(1기분)·`total - 1기분`(2기분, 반올림 오차 없이 합이 total과 정확히 일치)로 나눈다. `total`이 0원이면 `lumpSumEligible`도 false로 둬(낼 세금이 없는데 "일괄징수 대상"이라고 안내하지 않도록). `propertyTaxStepsHtml()`에 마지막 단계로 분납 안내 문장 추가(20만원 이하면 일괄징수 가능 안내, 아니면 두 기분 금액 명시). `propertyTaxPanelHtml()`의 합계 행 아래에 `lumpSumEligible`이면 안내 문구 한 줄, 아니면 1기분·2기분 두 행을 표시. |
| 검증 | 로직 테스트 151→155(4건 추가): 3억원 1세대1주택(합계 299,400원, 20만원 초과) -> 1기분·2기분 각 149,700원·`lumpSumEligible===false`; 1억원 일반주택(합계 156,000원, 20만원 이하) -> `lumpSumEligible===true`; 공시가격 0원 -> `lumpSumEligible===false`(0원을 일괄징수 대상으로 잘못 안내하지 않는지 확인하는 경계값 테스트); 실제 고지서 오라클(합계 1,514,876원) -> 1기분·2기분 각 757,438원, 합이 total과 정확히 일치. `node scripts/tests/logic.test.mjs` -> 155 passed, 0 failed. `npm run harness:check` 12/12 Required 통과(11 PASS + 1 수동). 헤드리스 Chromium 9/9: 3억원 1세대1주택 시나리오에서 `#ptFirstInstallment`·`#ptSecondInstallment`에 각각 149,700원이 표시되고 계산 과정에 "1기분"·"2기분" 문구가 등장함을 확인, 1억원 일반주택 시나리오(20만원 이하)에서는 두 분납 행이 숨겨지고 대신 "20만원"·"조례" 문구가 들어간 안내가 나타나며 계산 과정에도 "20만원"·"한꺼번에" 문구가 등장함을 확인. 콘솔 에러는 파비콘 404 하나뿐(앱 로직과 무관). |
| 정확성(North Star) | 사용자가 이미 실제 고지서를 통해 체감했던 "1기분/2기분" 구조를 그대로 계산기 숫자로 재현했다 — 계산 로직을 새로 만들지 않고 이미 검증된 `total`을 나누기만 해서 회귀 위험을 최소화했다. 법이 지자체 조례에 위임한 부분(20만원 이하 일괄징수, 그 기준에 지방교육세가 포함되는지)은 단정하지 않고 안내로만 남겨, "아직 확실하지 않은 것을 확정된 것처럼 보여주지 않는다"는 하드 룰을 지켰다. |
| 남은 위험/미완(0.47) | 20만원 이하 구간의 일괄징수 여부·기준 포함 범위(지방교육세 포함 여부)는 지자체 조례에 따라 다를 수 있어 이 앱이 자동 판정하지 않는다(안내 문구로만 노출) — 실제 지자체 조례 사례로 교차검증하지는 못했다. 기존 재산세 계산기의 남은 위험(지역자원시설세·토지·건축물 등 미구현, 1세대1주택 특례 한시 규정의 2027년 이후 연장 여부 미확인)은 그대로 유효. |
| 스킬 버전 | `Sub_app-research-notes_0.61` |

## 2026-07-15 앱 0.48: 지역자원시설세(소방분) + 공시가격 읽기 쉬운 표기 + 재산세 용어 설명

| 항목 | 내용 |
|---|---|
| app_version | 0.47 → 0.48 |
| note_type | `feature`(새 세목 계산 1건, UX 표기 1건, 문서화 1건 — 세 요청을 한 작업으로 처리) |
| 제목 | 사용자가 첨부한 계산기 화면 스크린샷(공시가격 입력칸에 콤마 없는 숫자 "1580000000"이 그대로 보임)을 근거로 세 가지 요청: (1) 공시가격을 회계단위로 파악하기 쉽게, (2) 지역자원시설세도 추가, (3) 계산 근거의 세법 용어를 쉬운 말과 취지·목적으로 설명 |
| 배경 | 0.47 배포 완료 보고 후 사용자가 실제 계산기 화면 스크린샷과 함께 세 요청을 한 번에 전달. "진행하자" 대신 바로 "진행"에 준하는 구체적 요청이라 별도 확인 없이 조사→구현 진행. |
| 조사 | **지역자원시설세(소방분)**: 지방세법 §141(목적: "지역의 부존자원 보호·보전... 소방사무에 소요되는 제반비용에 충당")·§142①3호(과세대상: "소방시설로 인하여 이익을 받는 자의 건축물(주택의 건축물 부분을 포함한다)")·§146③1호(6단계 누진세율표, 600만원 이하 10,000분의 4부터 6,400만원 초과 10,000분의 12까지)·§146④단서(주택 건물분 과세표준 = 지자체장이 산정한 건물 시가표준액 × §110①2호 공정시장가액비율, 재산세와 같은 비율)·§147②(과세기준일·납기는 재산세 §114·§115를 준용, 즉 소방분도 자체 세액 기준으로 20만원 이하 일괄징수 판정)를 원문 조회. 시행령 §138①1호로 "화재위험 건축물"(200%/300% 가산) 정의를 확인한 결과 "주거용이 아닌" 건축물만 해당해 주택은 항상 가산 없음을 확인 — 이 계산기가 주택만 다루므로 가산 로직이 필요 없다는 것을 법령으로 확정. 핵심 난점: 주택의 "건물분 시가표준액"은 공동주택가격(공시가격, marketValue)과는 다른 값으로 지자체장이 별도 산정하며, 국토부 realtyprice.kr에서 조회할 수 없다 — 위택스 시가표준액 조회 또는 작년 고지서의 지역자원시설세 항목에서 확인해야 하므로 이 값을 사용자가 직접 입력하는 선택적 필드로 설계(모르면 비워두면 이 세목만 생략, 기존 직전연도 공시가격 필드와 같은 패턴). **공시가격 읽기 쉬운 표기**: 기존 회계 화면 어디에도 실시간 콤마·억만원 변환 입력이 없어 새 패턴을 만들되(입력값 자체는 건드리지 않고 입력 아래에 별도 미리보기 줄만 추가) 기존 숫자 입력 UX를 깨지 않도록 최소 침습으로 설계. **용어 설명**: 기존 `TAX_TERMS`(세법 용어사전 SSOT, `term-ledger-contract` 게이트로 코드-문서 양방향 일치 강제)에 재산세 관련 용어가 하나도 없었음을 확인 — 새 카테고리 "재산세(지방세)"로 8개 추가(기존 "과세표준" 용어는 소득세법 개념이라 이름이 겹치면 `TermService.find()`가 첫 매치만 반환해 잘못된 정의를 보여주는 버그가 생기므로 "재산세 과세표준"으로 구분해 이름 충돌을 피함). |
| 구현 | `PropertyTax.FIRE_LEVY_BRACKETS`(6단계 누진공제표, §146③1호 원문에서 직접 유도) 신설. `calculateHousing()`에 `buildingValueForFireLevy` 파라미터 추가 — null/빈값이면 `fireLevyBase`/`wholeFireLevy`/`fireLevy`/`fireLevyFirstInstallment`/`fireLevySecondInstallment`/`fireLevyLumpSumEligible`을 모두 null로 반환(계산 생략). 값이 있으면 이미 계산된 `ratio`(이번 연도 공정시장가액비율)를 재사용해 과세표준을 구하고, 재산세와 같은 지분율(`share`)을 적용하며, §147②에 따라 소방분 자체 세액만으로 독립적인 20만원 일괄징수 판정과 반기분납 계산을 수행 — 재산세 `total`에는 합산하지 않고 별도 필드로 반환(과세표준이 다른 별개 세목이라 통합하면 오해 소지). `Utils.wonKorean(value)` 신설 — "15억 7,800만원"처럼 억/만원 단위로 읽어주는 순수 표기 함수(계산에는 관여하지 않음). 공시가격·직전연도 공시가격·건물 시가표준액 입력칸 아래에 `<div class="amount-echo">`를 추가하고 `ptRecalc`에서 입력마다 갱신. `renderPropertyTaxCalculator()`의 "계산 근거" 섹션에 `TAX_TERMS.filter(t => t.cat === '재산세(지방세)')`를 `termCardHtml()`로 렌더링하는 용어 카드 목록 추가(기존 세법 용어사전 페이지와 같은 렌더 함수 재사용, SSOT 중복 없음). `propertyTaxPanelHtml()`·`propertyTaxStepsHtml()`에 소방분 결과·계산 과정 문장 추가(건물 시가표준액 미입력 시 통째로 숨김). 상단 안내문의 "지역자원시설세(소방분)는 다음 업데이트에서 추가할 예정" 문구를 제거하고, 재산세와 별도 표시된다는 안내로 교체. |
| 검증 | 로직 테스트 155→170(15건 추가): 소방분 미입력 시 전부 null, 건물 시가표준액 3,000만원×1세대1주택 43%=1,290만원 과세표준→5,850원(20만원 이하, lumpSumEligible true), 5억원×일반 60%=3억원 과세표준→332,300원(20만원 초과, 1기분·2기분 각 166,150원), 지분 50% 적용 시 물건 전체(75,500원)의 절반(37,750원)만 남는지 확인. `Utils.wonKorean` 5건(1,578,000,000→"15억 7,800만원", 1,278,000,000→"12억 7,800만원", 3억원 정각→"3억원"(만원 단위 생략), 149,700→"14만 9,700원", 0→"0원") — 첫 테스트 작성 때 "15억 8,000만원"으로 잘못 기대했다가(1,580,000,000과 착각) 테스트 실행 중 발견해 즉시 정정했다. `TermService.all().length`를 31→39로 갱신하고 신규 용어 2건 조회 테스트 추가. `node scripts/tests/logic.test.mjs` → 170 passed, 0 failed. `npm run harness:check` 12/12 Required 통과(11 PASS + 1 수동, `term-ledger-contract`가 코드-문서 39개 양방향 일치 확인). 헤드리스 Chromium 12/12: 공시가격 1,578,000,000 입력 시 echo에 "15억 7,800만원" 노출, 건물 시가표준액 미입력 시 소방분 블록 전체가 없다가 입력하면 나타남, 계산 과정에 "지역자원시설세" 문구 등장, 용어 카드에 공정시장가액비율·지역자원시설세(소방분)·반기분납·"지방세법" 근거 표기가 모두 노출됨을 확인. |
| 정확성(North Star) | 소방분의 과세표준이 재산세와 다른 값(건물분 시가표준액)이라는 걸 조문(§146④단서)에서 직접 확인하지 않았다면, 공동주택가격을 그대로 재사용해 잘못된 세액을 보여줄 뻔했다 — "그럴듯해 보이는 재사용"을 법령 대조 없이 확정하지 않는다는 이 세션의 반복 원칙을 다시 지켰다. 20만원 일괄징수 판정도 재산세 합계와 소방분을 섞지 않고 §147②가 재산세 규정을 그대로 준용한다는 문언에 따라 소방분 자체 금액으로 독립 판정했다. `Utils.wonKorean` 테스트 작성 중 숫자를 잘못 암산했다가(1,578,000,000을 1,580,000,000으로 착각) 테스트 실행으로 즉시 걸러진 것도, "손으로 검산한 값"조차 실행해서 확인해야 한다는 원칙을 보여준 사례. |
| 남은 위험/미완(0.48) | 건물 시가표준액은 국토부 공시가격 알리미로 조회할 수 없어 위택스 조회나 작년 고지서 의존 — 이번 실제 고지서 사례(0.43~0.47에서 검증한 서초구 사례)엔 지역자원시설세 실측값이 없어 소방분 계산 자체는 오라클(실제 고지서) 대조를 하지 못했다(내부 정합성만 확인). 20만원 이하 일괄징수의 정확한 기준 범위(지방교육세 포함 여부 등)는 여전히 미확인. 재산세와 소방분을 실제 지자체가 통합 고지하는지 별도 고지하는지도 미확인(안내 문구로만 caveats 처리). 그 외 재산세 계산기의 기존 남은 위험(토지·건축물 등 미구현, 1세대1주택 특례 2027년 이후 연장 여부)은 그대로 유효. |
| 스킬 버전 | `Sub_app-research-notes_0.62` |

## 2026-07-15 앱 0.49: 상단바 가계부 빠른 전환

| 항목 | 내용 |
|---|---|
| app_version | 0.48 → 0.49 |
| note_type | `feature`(UX 진입점 추가, 계산·회계 로직 무변경) |
| 제목 | 대시보드 상단바(동기화 배지 옆 빈 공간)에 가계부 빠른 전환 버튼 신설 |
| 배경 | 사용자가 대시보드 화면 스크린샷에 "동기화 완료" 배지 왼쪽 빈 공간을 파란 원으로 표시하며 "가계부 또는 사업자를 선택해서 전환이 용이하게 하는게 어떨까요?"라고 제안. 탐색적 질문이라 짧게 추천·트레이드오프만 제시: 찬성(지금은 가계부 전환이 설정 화면 안에 있어 여러 개를 쓰는 사용자가 매번 여러 단계를 거쳐야 함), 트레이드오프(모바일 폭에서 상단바 공간이 이미 꽉 차 있어 이름 전체를 넣으면 overflow 위험 — 축약 표시+아이콘 버튼+팝업 전환 방식을 제안). 사용자 "네 진행하죠" 승인. |
| 조사 | 상단바 CSS(`@media (max-width: 780px)`)를 확인해, 현재 가계부 이름을 보여주는 `#topbarContext`가 모바일 폭에서 아예 `display:none`으로 숨겨져 있어 모바일에서는 지금 보고 있는 가계부가 화면 어디에도 안 보인다는 걸 발견 — 사용자가 지적한 "전환 어려움"보다 더 근본적인 gap(모바일에서 "지금 뭘 보고 있는지"조차 안 보임)이었다. 기존 `renderLedgerManagement()`(설정 화면의 가계부 관리 목록)와 `AppService.switchLedger()`를 그대로 재사용할 수 있음을 확인. `openModal()`/`closeModal()`과 `renderMonthlyUsageModal()`/`bindReviewEvents()` 같은 기존 "자체 바인딩 모달" 패턴(모달 콘텐츠는 `bindViewEvents()`가 아니라 자기 렌더 함수 안에서 직접 이벤트를 붙임)을 그대로 따르기로 함. |
| 구현 | `renderLedgerManagement()` 안에 있던 `modeLabel` 클로저를 최상위 순수 함수 `ledgerModeLabel(business)`로 추출(SSOT — 설정 화면 목록과 새 전환 모달이 같은 헬퍼를 공유, 로직 중복 없음). 상단바(`topbar-actions`, 동기화 배지 앞)에 `#ledgerSwitchButton`(아이콘 `repeat` + 현재 가계부 이름 라벨) 추가, 가계부가 하나도 없으면 `hidden`. 클릭 시 `renderLedgerSwitchModal()`이 가계부 목록을 모달로 띄우고(활성 가계부는 "사용 중" 배지, 나머지는 "전환" 버튼), 전환 클릭 시 기존 `AppService.switchLedger()`를 호출해 모달을 닫고 즉시 재렌더. 새 가계부 추가는 이 모달에서 중복 구현하지 않고 "가계부 관리로 이동" 버튼으로 설정 화면 폼을 그대로 안내(최소 변경). `renderShellStatus()`가 매 렌더마다 버튼 표시 여부와 라벨 텍스트를 갱신. CSS `.ledger-switch-button .button-label`에 `max-width`+말줄임(`text-overflow: ellipsis`)을 넣어 긴 가계부 이름도 상단바 레이아웃을 안 깨도록 함 — 모바일 폭에서는 기존 `.topbar-actions .button-label { display: none }` 규칙이 이 새 버튼에도 그대로 적용돼 아이콘만 남는다(새 브레이크포인트 규칙을 추가하지 않고 기존 패턴 재사용). |
| 검증 | 순수 로직 변경 없음(`ledgerModeLabel` 추출은 동작이 완전히 동일한 리팩터라 로직 테스트 대상 아님 — 기존 UI 렌더 함수 관례를 그대로 따름). 로직 테스트 170 유지(회귀 없음, `node scripts/tests/logic.test.mjs` 재확인). `npm run harness:check` 12/12 Required 통과. 헤드리스 Chromium 12/12: 가계부가 없는 첫 화면에서 버튼이 숨어 있다가 첫 가계부 생성 후 이름과 함께 나타남을 확인, 두 번째 가계부를 만들면(자동 전환) 버튼 라벨이 즉시 갱신됨을 확인, 전환 모달을 열어 두 가계부가 모두 나열되고 활성 가계부엔 "사용 중" 배지가 붙음을 확인, 비활성 가계부의 "전환" 버튼을 눌러 모달이 닫히고 상단바 라벨이 실제로 바뀜을 확인, 모바일 폭(390px)에서 버튼 라벨 텍스트는 숨고 아이콘만 남으며 버튼 자체는 여전히 보이고 가로 overflow가 없음을 확인. 콘솔 에러는 파비콘 404뿐. |
| 정확성(North Star) | 사용자가 지적한 지점(전환 불편)을 조사하다가 그보다 더 심각한 gap(모바일에서 현재 가계부가 아예 안 보임)을 코드에서 직접 찾아 함께 해결했다 — 요청받은 것만 좁게 처리하지 않고 관련 CSS·상태를 실제로 따라가 봤기 때문에 발견할 수 있었다. 새 목록·전환 로직을 중복 작성하지 않고 기존 `renderLedgerManagement()`/`AppService.switchLedger()`/모달 패턴을 그대로 재사용해 회귀 표면을 최소화했다. |
| 남은 위험/미완(0.49) | 새 가계부 추가는 이 모달에서 바로 할 수 없고 설정 화면으로 안내만 한다(의도적 — 폼을 두 곳에 중복 구현하지 않기 위함, 필요성이 확인되면 후속으로 인라인화 가능). 가계부가 아주 많아지면(예: 10개 이상) 모달 목록이 길어질 수 있으나 스크롤 가능한 기존 `task-list`/`modal-body` 구조를 그대로 써서 별도 대응은 하지 않음. |
| 스킬 버전 | `Sub_app-research-notes_0.63` |

## 2026-08-01 앱 0.50: 안드로이드 앱 설치 지원(Capacitor 얇은 셸 + 고정 링크 자동 갱신)

| 항목 | 내용 |
|---|---|
| app_version | 0.49 → 0.50 |
| note_type | `feature`(대형 — 새 하위 프로젝트 `android-shell/`, CI 워크플로, 웹앱 로그인 코드 분기 신설. 회계 도메인 로직은 무변경) |
| 제목 | 웹앱을 그대로 불러오는 얇은 Capacitor WebView 셸로 안드로이드에 설치 가능한 APK를 만들고, "항상 최신 APK 고정 링크 + 재설치 없는 자동 갱신"을 구현 |
| 배경 | 사용자가 상세 스펙(0단계 확인 사항 포함)을 제시하며 요청. 진행 중 사용자가 추가로 "가이드 안에 설치 안내를 초등학생도 쉽게 하고 최신 파일 다운로드까지, 그리고 업데이트되면 APK도 자동으로 업데이트되도록(기계화)"를 요청 — 두 요청을 한 작업으로 처리. |
| 0단계 확인(사용자 요청대로 구현 전에 먼저 답변) | (1) 배포된 고정 URL 있음(`https://hanwha27-tdtu.github.io/Accounting_ledger/`, 이 세션에서 여러 번 배포 확인). (2) 코드 전체에 GPS/EXIF/카메라 등 브라우저 권한만으로 못 하는 기능이 전혀 없음(증빙 업로드도 표준 `<input type=file>`→Cloudinary) — 네이티브 플러그인(스펙 4번) 스코프 자체를 건너뛰기로 결정. **다만 구현 중 이 답변에 대한 정정이 필요했다**: 이 앱의 유일한 로그인 방식인 Google OAuth는 구글 정책상 앱 내장 웹뷰 안에서 차단된다(`disallowed_useragent`) — GPS/EXIF와는 다른 카테고리지만, 셸 안에서 로그인 자체가 아예 안 되는 문제라 "네이티브 기능 불필요"와 별개로 시스템 브라우저(Chrome Custom Tabs) + 딥링크 처리가 사실상 필수임을 뒤늦게 발견해 구현에 포함했다(아래 참고). |
| 사용자와 함께 정한 결정 | AskUserQuestion으로 두 가지 확인: (1) appId/appName — `io.github.hanwha27tdtu.bareunjangbu` / "바른장부"(GitHub 계정 기반 역도메인, 별도 도메인 없이도 충돌 없음). (2) 서명 전략 — release keystore를 Claude가 직접 keytool로 생성해 전달하고 사용자가 GitHub Secrets에 등록하는 방식(추천안 채택). keystore·비밀번호는 Git에 절대 커밋하지 않고 세션 scratchpad에서 생성해 사용자에게 파일로 전달했다(`SendUserFile`), 저장소에는 흔적을 남기지 않음. |
| 구현 — 셸 프로젝트 | `android-shell/`(웹 리포와 같은 저장소, 별도 npm 하위 프로젝트 — CLAUDE.md의 "빌드 도구 없음" 규칙은 `index.html` 자체에만 적용됨을 문서에 명시). `npx @capacitor/cli`(v7) + `npx cap add android`로 표준 gradle 프로젝트 생성. `capacitor.config.json`의 `server.url`이 배포 주소를 그대로 가리켜(`webDir`는 빈 폴더) 웹 자산을 셸에 번들링하지 않는다 — 웹을 고쳐도 APK를 다시 굽지 않아도 된다는 핵심 계약. **알려진 한계를 솔직하게 문서화**: 스펙이 요구한 "`allowNavigation`을 호스트 전체가 아니라 경로로 좁혀라"는 Capacitor가 원리적으로 지원하지 않는다(그 설정은 호스트 단위로만 동작 — "다른 호스트로의 이동 허용" 목록이지 "같은 호스트 안 경로 제한" 기능이 아님을 확인). 즉 GitHub Pages 사용자 호스트(`hanwha27-tdtu.github.io`)에 이 계정의 다른 저장소가 Pages로 떠 있다면 그 페이지와 이 앱은 여전히 같은 origin이라 `localStorage`/`IndexedDB`를 서로 읽을 수 있는 구조적 위험이 있고, 이는 셸을 씌우기 전부터 있던 GitHub Pages 사용자 사이트 구조 자체의 한계라 `allowNavigation`으로 해결 불가능함을 `android-shell/README.md`에 명시(완전히 없애려면 전용 도메인/조직 Pages로 옮겨야 함, 별도 작업). |
| 구현 — 구글 로그인(딥링크) | `CapacitorShell`(index.html) — `window.Capacitor` 감지를 이 객체 한 곳에서만 한다. 셸 안에서는 `signInWithOAuth({ skipBrowserRedirect: true })`로 URL만 받아 `@capacitor/browser`(시스템 브라우저)로 열고, 구글→Supabase 콜백→커스텀 스킴(`io.github.hanwha27tdtu.bareunjangbu://auth-callback`) 딥링크로 돌아오면 `@capacitor/app`의 `appUrlOpen`을 받아 `exchangeCodeForSession`(PKCE)으로 세션을 완성한다. npm 패키지를 번들링하지 않는 이 앱 구조상, 플러그인 JS 래퍼 대신 셸이 주입하는 원시 브리지(`window.Capacitor.Plugins.Browser/App`)를 직접 호출 — 별도 CDN import나 빌드 없이 동작한다. `AndroidManifest.xml`에 이 커스텀 스킴 `intent-filter`를 수동 추가(`cap sync` 재실행해도 보존됨을 확인). **셸 밖(일반 브라우저)에서는 이 경로가 전혀 관여하지 않음**(`CapacitorShell.isNative()`가 항상 false). **사용자가 직접 해야 하는 유일한 설정**: Supabase 대시보드 → Authentication → URL Configuration → Redirect URLs에 위 커스텀 스킴을 추가해야 딥링크가 실제로 동작한다(구글 클라우드 콘솔 설정은 변경 불필요 — 구글은 항상 Supabase의 고정 콜백으로만 리다이렉트하고, 커스텀 스킴은 그다음 단계에서 Supabase가 처리). |
| 구현 — CI·서명 | `.github/workflows/android-apk.yml` — `android-shell/**` 경로 변경이 main에 반영될 때만 트리거(웹만 바뀐 커밋은 절대 안 돎). release keystore secret(`ANDROID_KEYSTORE_BASE64` 등 3개) 존재 여부를 확인해 있으면 `assembleRelease`로 서명 빌드, 없으면 `assembleDebug`로 폴백(사이드로드는 되지만 CI가 빌드마다 새 debug 키를 만들어 SHA가 바뀔 수 있다는 위험을 워크플로 주석과 README에 명시). `build.gradle`에 `signingConfigs.release`를 조건부로 추가(secret 없을 땐 참조 자체가 안 됨). 산출 APK를 고정 태그(`apk-latest`)에 `gh release upload --clobber`로 계속 덮어써서(새 태그 생성 안 함) 다운로드 링크가 영구히 고정되게 함. GitHub Actions 실행 번호(`github.run_number`, 항상 증가)를 `versionCode`로 주입해 셸 자체 갱신 감지에 사용(아래). |
| 구현 — SSOT + 검증 스크립트 | `index.html`의 `APK_INFO` 상수 하나가 릴리스 태그·자산 파일명·다운로드 URL의 유일한 출처. 워크플로(`android-apk.yml`)와 설치 안내 화면(`androidInstallGuideHtml()`)이 모두 이 상수만 읽는다. 사용자가 요청한 "세 곳 일치 검사 스크립트, 일부러 하나씩 깨뜨려서 실패하는지까지 확인"을 문서에 새 스크립트를 따로 만드는 대신 **기존 하네스 게이트 패턴**(`term-ledger-contract` 등과 같은 방식)으로 구현 — `scripts/harness-check.mjs`에 `apk-link-contract`(REQUIRED) 신설: (1) `APK_INFO`의 `releaseTag`/`assetName`/`downloadUrl`이 서로 모순 없는지(문자열에 실제로 포함되는지), (2) 워크플로의 `RELEASE_TAG`/`APK_ASSET_NAME`이 `APK_INFO`와 정확히 같은지, (3) 설치 안내 함수가 `APK_INFO.downloadUrl`을 실제로 읽는지(하드코딩 URL이 함수 밖에 따로 있는지도 검사). **실제로 하나씩 깨뜨려 검증**: 워크플로 태그를 바꿔치기 → FAIL 확인, `APK_INFO.assetName`을 바꿔치기 → FAIL 확인, 안내 화면에 URL을 하드코딩으로 치환 → FAIL 확인, 세 경우 모두 원상복구 후 다시 PASS + 파일이 바이트 단위로 원래대로 돌아왔는지 `diff`로 재확인. |
| 구현 — 웹 내용 자동 갱신(재설치 없음) | 빌드 단계가 없는 단일 HTML 구조라 스펙이 요구한 별도 `dist/version.json`을 만들지 않고, `index.html` 자기 자신을 `cache: 'no-store'`로 다시 받아 그 안의 `APP_INFO.version`만 정규식으로 읽어 비교(`WebUpdateService.parseVersion`/`isNewer`, 순수 함수로 분리해 로직 테스트 대상화). **입력 중 보호**: `keydown`이 아니라 `beforeinput`(capture)으로 "입력 중"을 감지 — 붙여넣기·음성입력·IME 조합·드래그 텍스트가 keydown 없이 들어오는 경우를 놓치지 않기 위함(스펙 요구사항 그대로). 이 플래그는 `visibilitychange`(화면 복귀)로는 리셋하지 않는다 — 글 쓰다 잠깐 나갔다 온 게 "안전"으로 둔갑해 새로고침이 입력을 날리는 사고를 막기 위해서다(스펙이 명시한 함정). 리셋은 오직 `markScreenTransition()`에서만 하는데, 이 앱은 History API 라우터가 아니라 해시 라우팅(`location.hash`+`hashchange`) 구조라 hashchange만 듣는 건 스펙이 지적한 문제(같은 라우트를 유지한 채 액션 후 재렌더되는 경우를 놓침)를 그대로 재현하므로, 이 앱의 유일한 화면 그리기 함수 `render()`(해시 이동이든 액션 후 재렌더든 전부 이걸 거침) 끝에서 직접 호출하도록 설계 — render()가 DOM을 막 새로 그린 시점은 입력 중이던 값이 이미 사라진 뒤라 그때 대기 중이던 갱신을 적용해도 추가 손실이 없다. 서비스워커는 이 앱에 아예 없어(검색 결과 0건) "SW가 version.json을 캐시하지 마라"는 주의사항은 해당 없음(문서에 명시). |
| 구현 — 셸(APK) 자체 자동 갱신("기계화" 추가 요청) | 사용자가 도중에 "APK도 자동으로 업데이트되도록 강제해줘(기계화)"를 추가 요청. **정직하게 확인하고 문서화한 기술적 한계**: 안드로이드는 플레이스토어를 거치지 않은(사이드로드) 앱이 스스로를 완전히 조용히(탭 0번) 덮어쓰는 것을 OS 보안 정책으로 막는다 — 이건 구현 미비가 아니라 넘을 수 없는 플랫폼 제약이라, "강제"를 문자 그대로 완전 무탭으로 구현하는 건 불가능함을 먼저 밝히고, 대신 안드로이드가 허용하는 한도 안에서 최대한 자동화했다: CI가 `github.run_number`를 셸의 `versionCode`로 주입하고, 같은 고정 릴리스에 작은 버전 매니페스트(`bareunjangbu-shell-version.json`, `--clobber`로 함께 덮어씀)를 추가로 올린다. 앱이 열리거나 화면에 복귀할 때(`CapacitorShell.checkForShellUpdate()`, 웹 내용 확인과 같은 타이밍) `App.getInfo()`로 설치된 자신의 versionCode를 이 매니페스트와 비교해, 더 새 빌드가 있으면 **다운로드까지는 자동으로 시작**(시스템 브라우저로 다운로드 URL을 염)하고 안드로이드의 "설치하시겠습니까?" 확인 화면(이 한 번은 어떤 앱도 건너뛸 방법이 OS 차원에서 없음)만 사용자가 누르면 끝나도록 만들었다. 같은 빌드를 반복 안내하지 않도록 `state.config.shellUpdatePromptedBuild`에 마지막으로 안내한 versionCode를 저장. `apk-link-contract` 게이트도 `versionAssetName`/`versionManifestUrl`까지 포함하도록 확장. |
| 구현 — 설치 안내(가이드) | `GUIDE_TOPICS`에 "안드로이드 앱 설치" 신설, 기존 `guideSection`/`guideValue` 패턴(구글클라우드 연결 가이드와 같은 형식) 재사용. 4단계: ①다운로드(버튼+주소, `APK_INFO.downloadUrl`) ②"차단된 파일"/"출처를 알 수 없는 앱" 경고가 뜨면 설정에서 허용하는 방법(초등학생도 따라 할 수 있는 순서로) ③설치 후 실행·로그인(구글 로그인이 앱이 아니라 별도 브라우저로 열린다는 것도 미리 안내해 사용자가 당황하지 않게 함) ④갱신은 두 가지(①화면·기능은 완전 자동 ②앱 자체는 자동 다운로드+설치 확인 한 번)로 나눠 정직하게 설명 — "완전 자동"이라고 과장하지 않음. |
| 검증 | 로직 테스트 170→182(+12) — `WebUpdateService.parseVersion`/`isNewer` 경계값(같은 버전은 false, 서버가 더 낮으면 false, null이면 false, x.99→(x+1).00 승급 비교 포함), `CapacitorShell.isNative()`가 Node/브라우저(둘 다 window.Capacitor 없음)에서 항상 false, `APK_INFO`의 URL들이 실제로 releaseTag/assetName/repo를 포함하는 자기 일관성. `npm run harness:check` 13/13(12 PASS + 1 수동, 신설 `apk-link-contract` 포함) — 세 곳 중 하나씩 깨뜨려 FAIL, 복구 후 다시 PASS까지 실제로 확인(요청받은 그대로). `npx cap add android`/`npx cap sync android`가 실제로 성공해 유효한 gradle 프로젝트가 생성됨을 확인(로컬에 Android SDK가 없어 실제 gradle 빌드 자체는 못 돌림 — CI 몫). 헤드리스 Chromium 10/10: 설치 안내 화면의 4단계 문구·다운로드 링크·배포기록 링크가 `APK_INFO`와 정확히 일치, `checkForWebUpdate()`가 같은 버전이면 조용히 끝나고 입력 중(dirty)일 때 더 최신 버전이 있어도 즉시 reload하지 않고 미루는지 마커 방식으로 확인, `CapacitorShell.isNative()`가 일반 브라우저에서 항상 false임을 확인. |
...
## 2026-08-01 앱 0.52: 0.51의 미검증 영역이었던 실기기 결과 — 모바일 메뉴 배경이 상단바 클릭을 가로채던 회귀 + 로그인 칩 무반응 수정

| 항목 | 내용 |
|---|---|
| app_version | 0.51 → 0.52 |
| note_type | `fix`(0.51에서 정직하게 "실기기 확인 필요"로 남긴 항목이 실제로 사용자에게 회귀를 일으킨 것으로 확인됨 — 내가 만든 버그를 내가 고침 + 곁들여 발견한 기존 UX 갭 1건 수정) |
| 제목 | 0.51 배포 직후 사용자가 "버튼들이 안 먹히고 로그인 필요 버튼 눌렀을 때 아무런 반응이 없어요"라는 스크린샷 제보 → z-index 스태킹 버그로 확인·수정, 곁들여 "로그인 필요" 표시가 애초에 클릭 불가능한 순수 텍스트였던 것도 함께 고침 |
| 배경 | 사용자가 0.51의 상태바·메뉴 배경 수정을 실기기에서 확인하던 중, 모바일 메뉴가 열린 상태의 스크린샷과 함께 제보. 스크린샷 자체가 0.51의 CSS 수정(어둡게 덮는 배경, `Accounting Ledger v0.51` 표시)이 실제로 반영됐음을 먼저 보여줘 0.51의 안전영역 여백 수정이 실기기에서도 정상 작동함은 이 스크린샷으로 간접 확인됨(상태바와 로고가 안 겹침). |
| 조사 — 원인 A(회귀, 내 버그) | 0.51에서 추가한 `#sidebarScrim`은 `position:fixed; inset:0`(화면 전체를 덮음)에 `z-index:35`인데, `.topbar`는 `z-index:30`이었다 — scrim이 topbar보다 위에 쌓여, 메뉴가 열려 있는 동안 사이드바 폭 바깥(화면 오른쪽, 상단바가 있는 영역)을 눌러도 그 터치가 밑에 있는 실제 버튼이 아니라 scrim에 먼저 닿아 메뉴만 닫히고 버튼 자체는 눌리지 않았다. 사용자가 스크린샷에서 "로그인 필요"를 누른 시점이 마침 메뉴가 열려 있던 상태라 이 문제를 바로 겪은 것으로 보인다. |
| 조사 — 원인 B(기존 UX 갭, 회귀 아님) | 상단바의 "로그인 필요" 표시(`#syncChip`)는 애초부터 `<div>`였고 클릭 이벤트 리스너가 코드 어디에도 연결돼 있지 않았다(`grep` 확인) — 로그인 자체는 설정 화면의 별도 `#googleLoginButton`에서만 가능했다. 시각적으로 옆의 "월별 사용현황"·"거래 입력" 버튼과 같은 높이·같은 pill 모양이라 사용자가 누르면 뭔가 반응할 거라 기대하는 게 자연스러운데, 실제로는 순수 상태 표시 텍스트였다 — "총체적 난국"이라는 인상을 준 원인 중 상당 부분이 여기서 왔을 것으로 판단. |
| 구현 | **A**: `.topbar`의 `z-index`를 `30`→`36`(scrim의 35보다 위, 사이드바의 40보다 아래)으로 올려 메뉴가 열려 있어도 상단바 전체(햄버거·로그인 칩·월별 사용현황·거래 입력)가 항상 scrim보다 위에서 클릭을 받도록 함. **B**: `#syncChip`을 `<div>`→`<button type="button">`으로 바꾸고(포커스·키보드 접근성도 함께 개선), 클릭 시 로그인 안 된 상태(`!state.session`)면서 로그인 가능한 조건(온라인+Supabase 설정됨+Google 제공자 비활성 아님)이면 기존 `#googleLoginButton`과 동일하게 `remote.signIn()`을 직접 호출(실패 시 기존 `diagnostic(error)` 패턴 재사용 — 새 에러 처리 안 만듦), 그 외 상태(로컬 전용·오프라인·Google 설정 필요·이미 로그인됨 등)면 설정 화면으로 이동(`routeTo('settings')`)해 자세한 상태를 보게 함. CSS는 `appearance: none`만 추가(기존 `.sync-chip` 규칙이 이미 border/background/radius를 다 지정해 버튼 기본 스타일을 충분히 덮음). |
| 검증 | 순수 CSS/DOM이라 로직 테스트 182 그대로. `npm run harness:check` 13/13(0.51→0.52 버전 증가 확인). 헤드리스 Chromium(390×844)으로 재현·검증: (1) 메뉴를 연 상태에서 `document.elementFromPoint()`로 syncChip 중심 좌표를 찍어 실제로 syncChip(자식 `#syncLabel`)이 잡히는지 확인(수정 전이라면 scrim이 잡혔을 자리) — 수정 후 정상적으로 syncChip이 잡힘을 확인. (2) syncChip을 실제로 클릭해 `remote.signIn()` 경로를 탔는지 확인 — 이 샌드박스는 외부 네트워크가 막혀 있어 로그인 자체는 실패하지만, 기존 `#googleLoginButton`과 동일한 `diagnostic(error)` 토스트("작업을 완료하지 못했습니다…")가 뜨는 것으로 올바른 코드 경로(routeTo가 아니라 signIn 시도)를 탔음을 간접 확인. |
...
## 2026-08-02 앱 0.54: 0.40~0.53 전수감사(자동 검사 + 헤드리스 E2E 34항목) + 안전영역 잔여 사각지대 마감

| 항목 | 내용 |
|---|---|
| app_version | 0.53 → 0.54 |
| note_type | `audit`(0.33~0.39 전수감사 이후 구간 전체를 재감사) + `fix`(감사에서 발견한 2건 수정) |
| 배경 | 사용자가 "지금까지 많은 기능들이 업데이트 했는데 전수감사 한 번 해보자"라고 요청. 0.40~0.53 구간은 재산세 계산기 확장(0.42~0.48), 상단바 가계부 전환(0.49), 안드로이드 셸 최초 구현과 실기기 검증 3라운드(0.50~0.53)가 몰려 있고, 특히 0.51~0.53은 긴급 수정을 연달아 배포하느라 각 수정의 파급을 차분히 훑을 틈이 없었다. |
| 감사 방법 — 정적 검사 | (A) 버전·문서 3종 교차: `UPDATE_HISTORY`·연구노트·handoff 표에 0.40~0.53 각 버전이 정확히 1회씩 존재, `최신 ·` 마커 1개, `APP_INFO.version` 일치 — 전부 정상. (B) ID 참조 무결성: `el('...')`·`getElementById` 호출 전수(고유 id 기준) vs `id="..."` 정의 전수를 diff — 누락 0. (C) 셸 봉합점: `window.Capacitor` 참조가 `CapacitorShell` 객체 밖에 0건. (D) z-index 스태킹 맵: scrim 35 < topbar 36 < sidebar 40 < modal 80 < tooltip 100 < toast 120 — 모달이 상단바 위, 토스트가 최상위로 의도대로. (E) 비밀값 스캔: keystore·비밀번호류 패턴 0건(하네스 게이트와 별도 이중 확인). (F) git 상태: 브랜치=main 동일 커밋, 작업 트리 깨끗. |
| 감사 방법 — 헤드리스 E2E(34항목) | 실제 브라우저(Chromium)에서 첫 화면부터 사용자 흐름 그대로: 모드 선택(2카드) → 사업자 모드 선택 → 사업자 폼으로 실제 생성 → **13개 라우트 전부** 진입해 렌더 크기·신규 에러 0 확인 → 거래 입력(필수값 포함) 저장 → "거래와 균형 전표를 저장했습니다. 차변·대변 11,000원" 토스트 → 장부·전표 화면 모두 표시 → **전체 새로고침 후에도 IndexedDB에서 복원**(영속 왕복) → 재산세 계산기(5억 입력 → 세액 산출 + "5억" 읽기 표기) → 가이드 4주제 전부 열림 + 안드로이드 설치 안내가 `APK_INFO.downloadUrl`을 그대로 표시(SSOT 일치) → 가계부 전환 모달 열림 → 모바일 뷰포트(390×844)에서 드로어 연 채 상단바 칩이 `elementFromPoint`로 실제 클릭을 받는지(0.52 회귀 재확인) + scrim 표시·탭 닫힘. 미처리 JS 에러 0, 샌드박스 네트워크 노이즈 외 콘솔 에러 0. |
| 감사 중 "실패"의 진상 | 초기 실행에서 유일하게 실패한 "거래 저장 왕복"은 스크립트가 필수 항목(계정과목)을 안 고르고 제출한 것으로, 앱이 "입력값을 확인해 주세요" 검증 토스트로 저장을 올바르게 막은 **정상 동작**이었다(계정과목 선택 후 재시도에서 전 과정 통과). scrim 클릭 타임아웃도 스크립트가 화면 중앙(드로어 폭 안쪽)을 클릭한 테스트 결함 — 실제 사용자 탭 위치(드로어 바깥)로 고치니 통과. 두 건 모두 "감사 도구의 실패 ≠ 제품 결함"임을 로그로 확인하고 진행했다. |
| 발견·수정 1: handoff "최근 기준 커밋" 행 부패 | `docs/claude-handoff.md`의 해당 행이 0.50 커밋 전 상태("34ce789, 0.49는 브랜치, 0.50 작업 중 커밋 전")로 남아 있었다 — 0.50~0.53의 4번의 배포 동안 상단 요약·표·백로그는 매번 갱신했지만 이 행만 누락. 현재 상태(0.53까지 main 배포, `apk-latest` release-signed 게시)로 갱신. 교훈: 이 행은 커밋 해시를 하드코딩하는 구조라 부패하기 쉽다 — 이번 갱신부터 해시 대신 "브랜치=main 동일" 같은 관계 서술로 바꿔 부패 표면을 줄였다. |
| 발견·수정 2: 안전영역 잔여 사각지대(모달·툴팁) | 0.51에서 상단바·사이드바·토스트에 `env(safe-area-inset-*)`를 넣었지만, 전수 스캔(`position: fixed` 전수 + `env(` 사용처 대조)으로 두 곳이 남아 있음을 확인: ① `.modal-backdrop`(패딩 20px 고정 — 안드로이드 15 edge-to-edge에서 `100vh`가 상태표시줄 영역을 포함하므로 최대 높이 모달의 상단이 시계에 겹칠 수 있음) ② 위로 뒤집힌 용어 툴팁(`Math.max(8, top)` 클램프가 화면 물리적 최상단 기준). 수정: 모달 백드롭 패딩 4방향에 안전영역 가산, `:root`에 `--safe-top: env(safe-area-inset-top, 0px)` 커스텀 프로퍼티를 두고 툴팁 클램프를 `Math.max(8 + safeTop, top)`으로(JS는 env()를 직접 못 읽으므로 CSS 변수를 경유). |
| 검증 | `npm run harness:check` 13/13(0.53→0.54 버전 증가 확인). 헤드리스로 모달 열림·중앙 배치와 용어 툴팁 표시가 수정 후에도 동일하게 동작함을 재확인(env()는 샌드박스에서 0px 폴백이라 수치 변화 자체는 실기기 확인 필요 — 단 0px 폴백에서 회귀가 없다는 것은 확인됨). |
| 정직하게 남기는 미검증 영역 | 모달·툴팁 안전영역 보완이 실기기에서 실제로 겹침을 없애는지는 사용자 확인 필요(다만 같은 기법이 0.51 상단바에서 실기기로 이미 검증된 상태라 위험 낮음). E2E는 로그인이 필요 없는 로컬 흐름만 검증 — 클라우드 동기화 왕복은 샌드박스 네트워크 차단으로 이번에도 검증 범위 밖(실기기에서 사용자가 로그인·동기화 완료를 확인해준 상태). |
| 남은 위험/미완(0.54) | 없음(감사 자체가 목적이었고 발견 2건 모두 같은 커밋에서 수정). 셸 자체 갱신(자동 다운로드+설치 확인) 흐름은 여전히 미트리거 상태 — `android-shell/**`가 다음에 바뀔 때 확인(체크리스트 10번). |
| 스킬 버전 | `Sub_app-research-notes_0.68` |

## 2026-08-02 셸 아이콘 교체(앱 버전 무변경): 기본 Capacitor 아이콘 → "BG장부" 브랜드 아이콘 — 셸 자동 업데이트 흐름 최초 실전 트리거

| 항목 | 내용 |
|---|---|
| app_version | 0.54 유지(`index.html` 무변경 — 버전 규칙상 셸 전용 변경은 버전을 올리지 않음) |
| note_type | `shell`(android-shell/** 전용 변경, 웹 코드·회계 로직 무관) |
| 배경 | 사용자가 홈 화면 스크린샷(기본 Capacitor 아이콘)과 함께 "아이콘을 장부에 어울리게 바꾸고 글자에 'BG장부'도 넣어달라" 요청. 4글자를 다 크게 넣으면 48px에서 뭉개지므로 "BG 크게 + 장부 작게" 구성을 추천하고 진행. |
| 디자인 | 앱 내부 브랜드와 동일 팔레트(진초록 `#163b33` 배경 + 금색 `#f4c45f` 마크 — index.html의 theme-color·brand-mark와 같은 값). 금색 라운드 사각을 장부 책 표지 모티브로 쓰고(하단에 원장 괘선 3줄), 표지에 진초록 "BG"(Noto Sans KR Black), 아래에 금색 "장부"(Bold). 한글 폰트는 샌드박스에 없어 npm(`@expo-google-fonts/noto-sans-kr`, 허용된 레지스트리)에서 TTF를 받아 PIL로 렌더 — 폰트 파일 자체는 Git에 추가하지 않음(빌드 시점 산출물인 PNG만 커밋). |
| 구현 | 마스터 1296px에서 그려 LANCZOS 축소로 15개 PNG 생성: 레거시 `ic_launcher`(라운드 사각 풀블리드)·`ic_launcher_round`(원형) 각 5밀도(48~192px), 어댑티브 `ic_launcher_foreground`(투명 배경, 콘텐츠를 safe zone 중앙 66% 안에 배치) 5밀도(108~432px). `values/ic_launcher_background.xml`의 어댑티브 배경색 `#FFFFFF`→`#163B33`. `mipmap-anydpi-v26/ic_launcher.xml`(어댑티브 정의)은 무변경 — 기존 참조 구조를 그대로 사용. |
...
## 2026-08-02 앱 0.57: 설치 플레이북 자동 발행 — 앱 가이드 화면이 SSOT, 파일은 스크립트가 생성, 게이트가 동기화 강제

| 항목 | 내용 |
|---|---|
| app_version | 0.56 → 0.57 |
| note_type | `feature`(문서 자동화) + `fix`(가이드 문구가 0.55 시절 동작을 설명하던 부패 1건을 이 작업이 즉시 적발·수정) |
| 배경 | 사용자 요청: "APK 설치 처음부터 끝까지 담은 플레이북도 제작, 앱 가이드에도 넣고, 코드가 바뀌면 자동으로 바뀌게 기계화, 앱 버전과 동일 버전으로 관리". 설치 안내 프로즈를 파일에 별도로 쓰면 앱 가이드와 두 벌이 돼 SSOT 중복 금지 규칙에 걸린다 — 대신 **방향을 뒤집어** 앱 안의 "안드로이드 앱 설치" 가이드(`androidInstallGuideHtml()`)를 유일한 원본으로 두고, 파일은 그 HTML에서 자동 생성하기로 결정. |
| 구현 | ① `scripts/build-install-playbook.mjs` — logic.test.mjs와 같은 방식으로 실제 앱 IIFE를 VM에 로드해 `androidInstallGuideHtml()`을 호출하고, 가이드 마크업 전용 소형 HTML→Markdown 변환기(guideSection→`## N.`, guide-steps→번호 목록, notice→인용문, guideValue→라벨+코드, 버튼→링크, 아이콘·복사버튼 제거)로 `docs/bareunjangbu-apk-install-playbook.md`를 생성. 헤더에 `v{APP_INFO.version}`을 그대로 물려받아 **앱 버전 = 플레이북 버전**이 구조적으로 보장됨. 출력은 결정적(타임스탬프 없음). `npm run playbook:build` 스크립트 등록. **구현 함정**: VM에 로드된 앱이 등록한 타이머가 이벤트 루프를 붙잡아 프로세스가 안 끝남 — `process.exit` 명시 종료 필수(logic.test.mjs가 이미 그렇게 하던 이유를 재발견). ② 하네스 REQUIRED 게이트 `install-playbook-sync` 신설 — 생성기를 `--check`로 돌려 파일과 바이트 비교, 어긋나면 FAIL(일부러 파일을 오염시켜 FAIL→복구→PASS 양방향 확인 완료). `project-contract`의 필수 파일 목록에 생성기·산출물 추가(생성기 삭제 시 조용히 무력화되는 구멍 차단). ③ 가이드 화면 맨 아래에 "이 안내는 플레이북 파일로도 자동 발행됩니다" 알림 추가(발행 관계를 사용자에게도 보이게). |
| 이 작업이 곧바로 잡은 부패 | 첫 생성 결과를 검수하다 가이드 4단계 ②가 **0.55 시절 동작("다운로드까지 자동으로 시작")을 설명**하고 있음을 발견 — 0.56에서 배너+버튼 방식으로 바꾸고 가이드 문구 갱신을 빠뜨렸던 것. 원본(가이드)을 배너 흐름("새 버전 앱이 나왔어요" 배너 → 업데이트 버튼 → 설치 확인 1탭, X로 닫으면 다음 실행 때 재안내)으로 고치고 재생성 — 정확히 이런 종류의 어긋남을 막으려고 만든 시스템이 만들어지는 도중에 첫 성과를 낸 셈. |
...
## 2026-08-02 거버넌스(앱 버전 무변경): 지침 파일 이원화를 구조적으로 제거 — CONSTITUTION.md → 어댑터 자동 생성 + adapter-parity 게이트

| 항목 | 내용 |
|---|---|
| app_version | 0.57 유지(`index.html` 무변경) |
| note_type | `governance`(SSOT 중복 클래스의 근본 제거 + 게이트 신설) |
| 배경 | 직전 작업에서 AGENTS.md↔CLAUDE.md 상충 3건을 손으로 찾아 고쳤는데, 사용자가 곧바로 더 근본적인 질문을 던짐: "상충되지 않도록 구조적·물리적으로 강제되어 있나?" 정직하게 아니라고 답했다 — 기존 `instruction-contract` 게이트는 "특정 키워드가 두 파일에 각각 존재하는가"만 볼 뿐 내용 모순은 전혀 못 잡고, 실제로 그 3건도 게이트가 아니라 사람이 읽어서 찾은 것이었다. 이어서 사용자가 해법까지 제시: "docs/CONSTITUTION.md 하나에서 두 어댑터로 자동 생성되고(gen:adapters), check-adapter-parity가 글자 단위 일치를 매 커밋 강제. 손으로 갈라지는 것이 애초에 불가능한 설계." |
| 설계 판단(사용자 제안에 대한 수정 1건) | 제안을 그대로 받되 한 가지를 바로잡았다 — **두 어댑터가 서로 바이트 동일할 수는 없다**(AGENTS.md는 동기화 12조·배포 8단계의 SSOT, CLAUDE.md는 버전 규칙·완료의 정의 SSOT라 담는 내용이 다름). 강제 대상은 *두 파일 사이*가 아니라 **헌법 → 각 생성 결과 사이**여야 한다. 그래서 규칙을 두 부류로 나눴다: ① 양쪽에 똑같이 있어야 하는 것(미션·스택·하드룰·스킬 라우팅) → 헌법에 한 번 쓰고 `emit:claude,codex` → 복제본 자체가 없으므로 갈라질 수 없다. ② 한쪽이 SSOT인 것 → 한쪽만 emit + 반대쪽엔 포인터 블록 → 위임이지 복제가 아니다. 두 어댑터를 100% 생성물로 만들어 "손으로 고치는 것"이 아예 불가능하게 했다. |
| 구현 | ① `docs/CONSTITUTION.md` — 18개 블록(`<!-- block:<id> emit:<audiences> -->`)의 단일 원본. 블록 밖 서문은 어디에도 emit되지 않는다. ② `scripts/build-agent-adapters.mjs` — 헌법을 파싱해 `CLAUDE.md`/`AGENTS.md`를 통째로 생성(`npm run gen:adapters`), `--check`로 바이트 비교. ③ 하네스 REQUIRED 게이트 `adapter-parity` 신설. ④ `project-contract` 필수 파일에 헌법·생성기 추가(삭제로 조용히 무력화되는 구멍 차단). 기존 `instruction-contract`는 제거하지 않고 2차 방어층으로 유지(헌법에서 블록이 통째로 사라지는 경우를 생성 결과 기준으로 잡는다). |
| 실패 모드 방어(설계의 핵심) | 이 시스템의 **가장 위험한 실패는 규칙이 조용히 사라지는 것**이다 — `emit:claud`처럼 한 글자만 틀려도 그 블록이 어댑터에서 소리 없이 빠진다. 그래서 생성기가 (a) 미지의 emit 대상, (b) 중복 block id, (c) emit 대상 0개, (d) 블록 마커 짝 불일치를 전부 **하드 에러**로 막는다. 구현 중 (d) 검사가 실제로 오탐을 냈다 — 헌법 서문에서 문법을 설명하려고 인용한 마커까지 세어버림 → 실제 블록은 항상 줄 맨 앞에서 시작하므로 정규식을 `^` 앵커 + `m` 플래그로 바꿔 해결(문서 가독성을 희생하지 않는 쪽). 파싱 오류는 스택 트레이스가 아니라 한 줄 메시지로 내보내 게이트 사유로 그대로 읽히게 했다. |
| 검증 | **일부러 3가지로 깨뜨려 각각 FAIL 확인 → 바이트 동일 복구 → PASS 재확인**: (1) CLAUDE.md만 손으로 수정(keystore 규칙 변조) → FAIL, (2) AGENTS.md만 손으로 수정(원본 세무자료 금지 변조 — 오늘 실제로 있었던 한쪽만 갈라지는 사고 패턴 재현) → FAIL, (3) 헌법의 emit 태그 오타 → 명확한 사유("emit 대상 \"claud\"을(를) 모른다 — 사용 가능: claude, codex")로 FAIL. 생성 전후 diff를 전수 검토해 유실 내용이 없고 차이가 전부 의도한 통합임을 확인(AGENTS의 push 승인 규칙은 공통 하드룰로 흡수, 나머지는 양쪽 최신본으로 수렴). `npm run harness:check` 15/15(14 PASS + 1 수동). |
