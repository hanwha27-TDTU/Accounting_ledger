...
모든 기능·설계·코드·리팩터링은 **"이게 그 목적을 더 잘 이루는가?"** 하나로 판단한다. 판단이 갈리면 이 미션이 나침반이다.

### 비타협 원칙 (North Star, 목적의 일부)

아래는 목적을 위해 "희생할 대상"이 아니라 목적을 이루는 **구성요소**다. 어떤 기능·속도·편의도 이걸 깎아 얻지 않는다.

1. **사실 정확성**: 세무·회계·법령 판단은 확인된 근거와 버전을 남긴다. 추정을 확정처럼 표시하지 않는다.
2. **데이터 무결성**: 복식부기 균형, 삭제=soft delete+tombstone, 다기기 병합 무손실(부활·소실 없음), 빈 클라우드 가드. 사용자 데이터를 조용히 잃지 않는다.
3. **정직한 완료**: 미구현을 완료처럼 보이게 하지 않는다. 실행하지 않은 검사를 통과했다고 하지 않는다. 실패·미검증·수동 확인 필요를 구분해 보고한다.
4. **보안·권한**: RLS·owner allowlist·비밀키 금지를 약화하지 않는다.

이 저장소의 나머지 규칙·계약·게이트(`AGENTS.md`, `CLAUDE.md`, 하네스, 스킬, 생명주기 매트릭스)는 전부 이 North Star를 섬기는 수단이다.
...
| 11 | 사업자/사업장 | 구조는 복수 지원, UI는 1개 중심 | 개인사업자 1개 사업장 UI(0.36에서 복수 가계부 전환 UI로 조기 확장 — 설정의 "가계부 관리") | 사업장(한 가계부 내 복수 사업장) UI |
...
| 항목 | 1차 포함 |
|---|---|
| 현재 UI | 가계부(사업자) 여러 개 생성·전환 가능(0.36, 설정 "가계부 관리"). 가계부당 사업장은 1개 중심 |
