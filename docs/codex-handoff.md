...
> 개정 2026-08-08 · 작업 트리 앱 버전 `0.71` · 공개 배포 상태는 Git 최신 커밋과 GitHub Actions가 SSOT
...
- **작업 릴리스는 0.71**이다. 0.70에서 추가한 진단 허브(5그룹·9도구, 저장·레코드·전표·동기화·백업·증빙·연결/RLS·런타임)의 "레코드 필수 필드" 도구가 append-only `audit_logs`(updated_at·deleted_at 없음)를 다른 저장 영역과 똑같이 취급해, 실사용으로 감사로그가 쌓일수록 오탐 "문제" 건수가 늘던 결함을 실기기 진단 보고서로 발견해 고쳤다(`recordContractIssues` 순수 함수로 분리, `audit_logs`만 면제). 0.69의 증빙 원본 독립 보관과 0.68의 원자 JSON backup 0.04·cloud-first LWW·canonical CAS는 그대로 유지한다. 로직 285 assertions, 실제 Chromium 왕복 24 assertions가 실행되며 전체 Required는 20개다. schema 0.08·backup 0.04는 그대로이고 migration은 없다.
...
- **0.59~0.62 반복지출·다중통화 아크**: 상단 고정지출현황과 실제 비용거래 연결 후, 거래·고정지출·추계소득·재산세에 KRW·UZS·USD·JPY 선택과 CBU 일일환율을 추가. 0.61에서는 반복 납부 예정일을 자동 계산했고, 0.62에서는 고정지출의 사용자 환율 기준일·실제 고시일 구분과 다중통화 금액 정렬을 완성했다. 저장 원장은 KRW로 유지하고 원금액·환율 감사필드를 보존하며 schema 0.06을 그대로 사용한다.
