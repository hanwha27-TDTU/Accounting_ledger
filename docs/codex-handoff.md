...
> 개정 2026-08-04 · 기준 앱 버전 `0.61` · 배포 상태는 Git 최신 커밋과 GitHub Actions가 SSOT
...
- **기준 앱은 0.61**(전 금액 입력 KRW·UZS·USD·JPY 선택과 CBU 일일환율, 월간 결제일·연간 결제월일 기반 반복 일정 자동 계산)이며 자동 테스트 기준은 로직 233 assertion + 하네스 17개(16 REQUIRED + 1 MANUAL)다. 운영 DB migration 0.06은 적용 완료됐으며 현재 웹 배포 여부는 Git 최신 커밋과 GitHub Pages를 확인한다.
...
- **0.59~0.61 반복지출·다중통화 아크**: 상단 고정지출현황과 실제 비용거래 연결 후, 거래·고정지출·추계소득·재산세에 KRW·UZS·USD·JPY 선택과 CBU 일일환율을 추가. 0.61에서는 월간 결제일·연간 결제월일을 기준으로 다음 납부 예정일을 자동 계산하고 월말·윤년·조기결제·미납·재개 경계를 고정했다. 저장 원장은 KRW로 유지하고 원금액·환율 감사필드를 보존하며 schema 0.06을 그대로 사용한다.
